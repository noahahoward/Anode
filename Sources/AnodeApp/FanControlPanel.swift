...
final class FanControlPanel: NSView {
    ...
    /// How many fan rows are on screen. For tests: "is there a control surface
    /// here" is the whole question this panel got wrong, and it is answerable
    ...
    private func render() {
        ...
        syncToggle.isHidden = !installed || usable.count < 2 || FanGauge.sharedLimits(usable.map {
        ...
        // NO CONTROL INTERFACE WITHOUT THE HELPER.
        //
        // Not a disabled one, and not a read-only bar either — both were tried.
        // A row of sliders is a control surface, and with nothing behind it the
        // only honest amount to show is none: the Fans tab already reports every
        // fan's speed above this strip, so the rows were carrying no reading that
        // is lost by removing them. What is left in this state is a sentence
        // saying macOS is in charge and the button that changes that.
        rebuildFanRows(installed ? fans : [])

        switch session.mode {
        case .off:
            // Whether there ARE sliders depends on the helper, so the sentence
            // has to follow. "Take one to turn it on" was an instruction that did
            // nothing on a machine with no helper — which is the machine most
            // likely to be reading it — and a later draft promised "bars" that
            // the same change had just removed. A caption describing controls
            // that are not on screen is worse than no caption.
            statusLabel.stringValue = installed
                ...
                : "Fan control is off — macOS is deciding. Install the fan "
                + "helper to take control of the fans."
    ...
    private func applyReading(_ f: FanInfo) {
        ...
        // it is how a user asks for control in the first place. These rows only
        // exist once the helper does, so the grab always reaches a consent sheet
        // and never an installation.
